...
function json.decode(str)
 ...
 --[[ 
   Importer only works for 
              Animation data that is stored in a json file with a meta.frameTags property(e.g)
              {
                "meta": {
                  "frameTags": [
                       { "name": "walk_down", "from": 0, "to": 2, "direction": "forward" },
                       { "name": "walk_left", "from": 3, "to": 5, "direction": "forward" },
                  ]
                }
              }

  
  This script will import existing animation tags you created for other sprites. Very useful if you don't want to manually set up tags on spritesheets of the same structure
...
dlg:file{ id="picker",
          ...
          onchange=function() 
            ...
            local new_tag = sprite:newTag(from, to)
            new_tag.name = name
